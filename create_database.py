...
############################################################################################################
    ...
    def create_dataframe(self, save=False):
        ...
        if self.dataset == '20NG':
            ...
            print(f'{self.dataset}, not available.')

        elif self.dataset == 'BBCNews':
...
############################################################################################################
    ...
    def generate_document_embeddings(self, save:bool = False):
        '''
        ...
        # doc_emb = self.docs_df['docs_clean'].progress_apply(lambda x: self.tf_model.encode([x]))
    ...
    def dataset_target_name_emb(self):
        ...
        if self.dataset == 'BBCNews':
            target_emb = self.tf_model.encode(self.docs_df['target'].apply(lambda x: self.clean_text(str(x))).unique())
    ...
    def calculate_classification_accuracy(self, topic_emb):
        ...
        if(self.dataset == 'BBCNews'):
            target_names = self.docs_df.target.unique().tolist()
            target_names_emb = self.dataset_target_name_emb()
            classifier_evaluation_matrix = self.most_similar_topics(self.calculate_similarities(v1=target_names_emb, v2=topic_emb), type='euclid')
            self.docs_df['target_name_projection'] = self.docs_df.target.apply(lambda x: classifier_evaluation_matrix[target_names.index(x)])
...
############################################################################################################
    ...
    if (args.process == 'build'):
        ...
        print(f'Classification Evaluation for a range of top N number of Similar Words.')
